...
func (*AliyunOSS) UploadFile(file *multipart.FileHeader) (string, string, error) {
	...
	if err != nil {
		global.LOG.Error("function AliyunOSS.NewBucket() Failed", zap.Any("err", err.Error()))
	...
	yunFileTmpPath := global.CONFIG.AliyunOSS.BasePath + "/" + "frame-web" + "/" + time.Now().Format("2006-01-02") + "/" + uuid + fileExt
	// yunFileTmpPath := filepath.Join("uploads", time.Now().Format("2006-01-02")) + "/" + file.Filename
	//yunFileTmpPath := global.CONFIG.AliyunOSS.BasePath + "/" + "frame-web" + "/" + time.Now().Format("2006-01-02") + "/" + file.Filename
